...
class App extends Component {
  ...
  matchingImageId = id => {
    ...
    const isImageMatch = mainImageId === id

    if (isImageMatch) {
      this.setState(prevState => ({
        clickedCardList: [...prevState.clickedCardList, id],
      }))
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
      this.setState({
        mainImageUrl: imagesList[randomImage].imageUrl,
        mainImageId: imagesList[randomImage].id,
      })
    } else {
      this.setState({isGameInProgress: false})
      clearInterval(this.timeInterval)
  ...
  getFilterProject = () => {
    ...
    const filterProject = imagesList.filter(
  ...
  getRenderFilterItem = () => {
    ...
    return (
      <>
        {filterProject.map(eachProject => (
          <FilterImage
            key={eachProject.id}
            projectDetails={eachProject}
            matchingImageId={this.matchingImageId}
          />
        ))}
      </>
    )
      ...
      <>
        {tabsList.map(eachTab => (
          <TabList
            ...
            updatedActiveTab={this.updatedActiveTab}
          />
        ))}
      </>
    )
  ...
  onReset = () => {
    ...
    this.setState({clickedCardList: []})
    this.setState({score: 0})
  }

  getRenderScoreBoard = () => {
    const {clickedCardList} = this.state
  ...
  render() {
    const {clickedCardList, timer, isGameInProgress, score} = this.state

    return (
      <div className="app-container">
        <Header
          clickedCardList={clickedCardList.length}
          timer={timer}
        ...
        {isGameInProgress ? (
          <>
            <div className="app-home-container">
              <ul className="active-image-container">
                {this.getRenderActiveImage()}
              </ul>
              <ul className="tab-list-container">{this.getRenderTabList()}</ul>
              <ul className="filter-project-container">
                {this.getRenderFilterItem()}
              </ul>
            </div>
          </>
        ) : (
          this.getRenderScoreBoard()
        )}
      </div>
    )
  }
